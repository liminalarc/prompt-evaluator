...
/**
 * A version's lifecycle badges (1.16): Current-in-source, the single **Backport target** (the one
 * highest-scoring version to ship — not every version that beats Current), and regressed. Independent
 * flags → zero or more badges. Colors come only from the brand tokens.
 */
export function versionStatusBadges(status: {
  ...
  if (status.isBackportTarget) badges.push({ variant: 'success', label: 'Backport target' });
